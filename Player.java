...
public class Player
{
  ...
  boolean BeginTurn(Scanner scan, Player previous_player)
  {
    ...
    if (previous_player != null)
    {
      ...
      // CHECK: player does NOT accuse previous player of lying   
      else if (choice == 0)
      {
        this.real_number = RollDice();

        System.out.println(this.name+", please hide the screen from the other players to prevent them from seeing your numbers. Confirm by entering anything and pressing enter.");
        scan.next();
        ...
        if (findInArray(this.real_number, ranks) >= findInArray(previous_player.communicated_number, ranks))
        {
          ...
          } while (!(findInArray(choice, ranks) < findInArray(previous_player.communicated_number, ranks)) || choice != 99);
        }
  ...
  public static int getPlayerIntInput(Scanner scan, int[] options)
  {
    int input = 0;
    do
    {
      try
      {
        ...
        if (input == 99 || findInArray(input,options) != -1)
        {
          return input;
        }
        else
